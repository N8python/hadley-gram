canvas = document.getElementById('hadleyGram');
...
Confetti.prototype.draw = function() {
  ...
  if (this.ball) {
    ...
    ctx.drawImage(aLoon, this.x+35, this.y+10);
    this.y -= this.yVel;
    this.yVel += 1;
  } else if (this.tiger) {
    ctx.drawImage(eLoon, this.x, this.y);
    this.y -= this.yVel;
    this.x += this.xVel;
    this.yVel -= 1;
  } else {
...
function animation() {
  ...
  if (firstFrame < 1401) {
    ...
    ctx.fillStyle = "Red";
    ctx.fillText("Hadley", 20 + firstFrame, 100);
    ctx.fillStyle = "Orange";
    ctx.fillText("Hads", 1200 - firstFrame, 200);
    ctx.fillStyle = "Green";
    ctx.fillText("Hadster", 20 + firstFrame, 300);
    ctx.fillStyle = "Blue";
    ctx.fillText("❤️Hadley❤️", 1200 - firstFrame, 400);
